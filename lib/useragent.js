...
function parse (useragent, jsagent) {
  // check for cache hit
  var ua;
  if (parse.cache && (ua = parse.dictionary[useragent])) return ua;
  ...
  var agent = new Agent(family, major, minor, patch, osys);
  if (!parse.cache) return agent;
  return parse.dictionary[useragent] = agent;
}
